...
exports.PollBot = class PollBot {
            ...
            timedPolls.forEach(p => {
                ...
                const editMessage = m => {
                    if (Date.now() > p.timedPoll) {
                        ...
                        embed.description = `**${p.question}**\n\n${answers.map((a, i) => `${/^\d+$/.exec(emojis[i]) ? this.client.emojis.get(emojis[i]).toString() : emojis[i]} | ${a} (${reactionCounts[i]})`).join("\n")}\n\n__Győztes__: ${answers[reactionCounts.indexOf(Math.max.apply(Math, reactionCounts))]}`;
                        embed.footer.text = `Többszörös szavazatok: ${p.blockMultiples === "true" ? "nem" : "igen"}, Vége a szavazásnak!`;
                        ...
                        embed.footer.text = `Többszörös szavazatok: ${p.blockMultiples === "true" ? "nem" : "yes"}, Vége: ${this.formatMs(p.timedPoll - Date.now())}`;
    ...
    formatMs(ms) {
        ...
        return `${days > 0 ? `${days} nap ` : ""}${hours > 0 ? `${hours} óra ` : ""}${minutes > 0 ? `${minutes} perc ` : ""}${seconds > 0 ? `${seconds} másodperc! ` : ""}`.trim();
    };
};
